...
class CollectedPhrase:
    ...
    def set_key (self, compound_key):
        ...
        """
        self.key = tuple(list(compound_key))
...
class TextRank:
    ...
    def link_sentence (self, sent):
        ...
        for i in range(sent.start, sent.end):
            ...
            if token.pos_ in self.pos_kept:
                ...
                # ...otherwise proceed
                key = (token.lemma_.lower().strip(), token.pos_,)
                ...
                if self.logger:
                    self.logger.debug(" -- {} {} {} {} {} {}".format(
                        token.i, token.text, token.lemma_.lower().strip(), token.pos_, visited_tokens, visited_nodes
                    ))
    ...
    def collect_phrases (self, chunk):
        ...
        for i in phrase.range():
            ...
            if key in self.seen_lemma:
                ...
                if self.logger:
                    self.logger.debug(" {} {} {} {}".format(
                        token.lemma_.lower().strip(), token.pos_, node_id, rank
                    ))
            else:
                phrase.non_lemma += 1
    ...
    def calc_textrank (self):
        ...
        for phrase_key, phrase_list in self.phrases.items():
            ...
            phrase_text = ' '.join(key[0] for key in phrase_key)
            min_phrases[phrase_text] = (best_phrase.rank, len(phrase_list), phrase_key)
